...
if(isset($auth) || isset($_SESSION['auth']) && $_SESSION['auth']['idgroup'] ==5){
	...
	array_map(function($index){
		global $method;
		$opacity = utf8_encode(str_replace(' ','-', $index->getName()))!='Packagings'?'opacity6':'';
...
if(isset($auth) || isset($_SESSION['auth']) && $_SESSION['auth']['idgroup'] ==5){
	...
	echo '<form class="margin-auto padding10 zindex2 bg-emerald" id="form_add" method="post" action="">';
	...
	echo '<p class="input-focus padding10"><label for="visuel"> Votre visuel :</label><input type="text" id="visuel" name="visuel" class="dracula-orchid" placeholder="visuel" required > *</p>';
	echo '<p class="input-focus padding10"><label for="cost"> Votre cout :</label><input type="text" id="cost" name="cost" class="dracula-orchid" placeholder="cost"> </p>';
	echo '<p class="input-focus padding10"><label for="transportcost"> Votre transportcost :</label><input type="text" id="transportcost" name="transportcost" class="dracula-orchid" placeholder="transportcost"> </p>';
...
asyncawait('http://localhost/macata/core/app/src/request/packaging.admin.request.php').then(function(response){
		document.getElementById('show_request').innerHTML = response;
	});
button_add.addEventListener("click",function(e) {
	let reference = document.getElementById('reference').value;
	let visuel = document.getElementById('visuel').value;
	let cost = document.getElementById('cost').value;
	let transportcost = document.getElementById('transportcost').value;
	let datecreation = document.getElementById('datecreation').value;
	let datas = `reference=${reference}&visuel=${visuel}&cost=${cost}&transportcost=${transportcost}&datecreation=${datecreation}`;
	asyncawaitpost('http://localhost/macata/core/app/src/request/packaging.admin.request.php',datas).then(function(response){
